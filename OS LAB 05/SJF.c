...
static void push_segment(Segment segs[], int *segc, const char *label, int s, int e) {
    ...
    segs[*segc].start = s; segs[*segc].end = e;
    (*segc)++;
}
...
static void print_table(Proc p[], int n) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        printf("%-10s %-4d %-4d %-4d %-4d %-10s %-4d %-10s\n",
               pid, p[i].at, p[i].bt, p[i].ct, p[i].tat, tatExpr, p[i].wt, wtExpr);
    }
    printf("\nAverage TAT = %.2f\n", avgTAT / n);
...
static void sjf_nonpreemptive(Proc p[], int n) {
    ...
    while (done < n) {
        ...
        t += p[idx].bt; p[idx].ct = t; p[idx].done = 1; last_ct = t; done++;
    }
...
static void sjf_preemptive(Proc p[], int n) {
    ...
    while (done < n) {
        ...
        if (idx == -1) {
            if (current != -1) {
                char label[16]; snprintf(label, sizeof(label), "P%d", p[current].id);
                push_segment(segs, &segc, label, seg_start, t); current = -1;
            }
            int nextAT = INT_MAX;
            for (int i = 0; i < n; ++i) if (p[i].rem > 0 && p[i].at > t && p[i].at < nextAT) nextAT = p[i].at;
            push_segment(segs, &segc, "idle", t, nextAT); t = nextAT; seg_start = t; continue;
        }
        if (current != idx) {
            if (current != -1) {
                char label[16]; snprintf(label, sizeof(label), "P%d", p[current].id);
        ...
        p[current].rem--; t++;
        if (p[current].rem == 0) {
            p[current].ct = t; p[current].done = 1; done++;
            char label[16]; snprintf(label, sizeof(label), "P%d", p[current].id);
            push_segment(segs, &segc, label, seg_start, t);
            current = -1; seg_start = t; if (t > last_ct) last_ct = t;
        }
    }
...
int main(void) {
    ...
    if (choice == 2) sjf_nonpreemptive(p, n); else sjf_preemptive(p, n);
    return 0;
}
